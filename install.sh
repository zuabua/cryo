...
NVCONF="$HOME/.config/hypr/nvidia.lua"
...
-- opted out). This stub keeps hyprland.lua's `require("nvidia")` valid.
...
for f in hyprland.lua binds.lua monitors.lua workspaces.lua vars.lua; do
  if [ -f "$HYP_DIR/$f" ]; then
    smoke_ok "$f present"
  else
...
HYP_CONF="$HYP_DIR/hyprland.lua"
if [ -f "$HYP_CONF" ]; then
  if [ -f "$HYP_DIR/vars.lua" ] && grep -qE '^[[:space:]]*mainMod[[:space:]]*=' "$HYP_DIR/vars.lua"; then
    smoke_ok "mainMod is defined"
  else
    smoke_fail "mainMod not defined in vars.lua — every bind in this config will be invalid"
  fi

  # exec_cmd + bind script paths must resolve. Lua config is split across
  # several files, so scan all of them rather than just the entrypoint.
  HYP_BAD=0
  while IFS= read -r path; do
    [ -z "$path" ] && continue
    expanded="${path/#\~/$HOME}"
    ...
    grep -hoE '~/[^ "'\'',;)]+' "$HYP_DIR"/*.lua 2>/dev/null | sort -u
  )
  if [ "$HYP_BAD" -eq 0 ]; then
    smoke_ok "all exec_cmd + bind script paths resolve"
  else
    smoke_fail "$HYP_BAD Hyprland script path(s) broken"
  fi

  # require()d modules must exist alongside the entrypoint. nvidia is
  # loaded via pcall and is optional, so it is not required to be present.
  SRC_BAD=0
  while IFS= read -r mod; do
    [ -z "$mod" ] && continue
    [ "$mod" = "nvidia" ] && continue
    if [ ! -f "$HYP_DIR/$mod.lua" ]; then
      warn "             missing module: $mod.lua"
      SRC_BAD=$((SRC_BAD + 1))
    fi
  done < <(
    grep -hoE 'require\("[A-Za-z0-9_-]+"\)' "$HYP_DIR"/*.lua 2>/dev/null |
      sed -E 's/require\("(.*)"\)/\1/' | sort -u
  )
  if [ "$SRC_BAD" -eq 0 ]; then
    smoke_ok "all Hyprland require() modules resolve"
  else
    smoke_fail "$SRC_BAD Hyprland require() module(s) missing"
  fi

  if grep -qE 'hl\.dsp\.focus\(\{[[:space:]]*workspace' "$HYP_DIR"/*.lua; then
    smoke_ok "workspace binds present (Super+1..0 should work)"
  else
    smoke_fail "no workspace binds found — config may be truncated"
  fi

  # The authoritative check: Hyprland parses the config itself.
  if command -v Hyprland >/dev/null 2>&1; then
    if HYP_ERR="$(Hyprland --verify-config -c "$HYP_CONF" 2>&1)"; then
      if printf '%s' "$HYP_ERR" | grep -q 'config ok'; then
        smoke_ok "Hyprland --verify-config: config ok"
      else
        smoke_fail "Hyprland --verify-config reported errors:"
        printf '%s\n' "$HYP_ERR" | sed -n '/Config parsing result/,$p' | sed 's/^/             /'
      fi
    else
      warn "             could not run Hyprland --verify-config"
    fi
  fi
fi
...
The repo is deployed. A few items still need your hands:
  ...
  ${C_TEAL}8. Universal inbox${C_RESET}
     ...
     line to ~/Documents/obsidian/inbox.md, Esc cancels.
